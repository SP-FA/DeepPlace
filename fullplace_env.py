...
def write(res, max_width, max_height):
        ...
        for line in f2:
            ...
            if line and l[0][0] == 'o':
                ...
                if num - 450927 in dic.keys():
                    ...
                    pos = res[index]
                    x = int(pos[0] / 32 * max_width)
                    y = int(pos[1] / 32 * max_height)
                    l[1] = str(x)
                    l[2] = str(y)
                    line = '\t'.join(l)
...
class Placememt():
    def __init__(self, grid_size=32, num_cell=710):
        ...
        benchmark = "adaptec3"
        node_info, netlist, max_width, max_height = generate_db_params(benchmark)

        # f = open("./DeepPlace/data/n_edges_710.dat", "r")
        # for line in f:
        #     self.net = eval(line)
        
        self.steps = len(node_info)
        self.net = netlist
